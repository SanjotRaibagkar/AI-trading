...
def detect_bearish_reversal(train_data,test_data):
    recco_dict = {}
    medium_term_trend = vasu.get_medium_trend(train_data)
    ...
    if medium_term_trend != 'bullish' or medium_term_trend != 'indeterminate':
        if peak_type == 'low' and peak_low_slope <= 0.0:
            recco_dict['medium_trend'] = medium_term_trend
            recco_dict['latest_peak'] = peak_type
            recco_dict['peak_slope'] = peak_low_slope
            recco_dict['trade'] = 'buy'
            recco_dict['benchmark_date'] = peak_date
            benchmark_price =  peak_data['High Price'].values[0]
            
            recco_dict['benchmark_price'] = benchmark_price
            
            signal_data = test_data[test_data['Close Price'] > benchmark_price][1:2].copy()
            if len(signal_data) > 0:
                recco_dict['entry_date'] = signal_data.index.to_pydatetime()[0]
            
    return recco_dict

def detect_bullish_reversal(train_data,test_data):
    recco_dict = {}
    medium_term_trend = vasu.get_medium_trend(train_data)
    ...
    if medium_term_trend != 'bearish' or medium_term_trend != 'indeterminate':
        if peak_type == 'high' and peak_high_slope >= 0.0:
            recco_dict['medium_trend'] = medium_term_trend
            recco_dict['latest_peak'] = peak_type
            recco_dict['peak_slope'] = peak_high_slope
...
def detect_bullish_continuation(train_data,test_data):
    recco_dict = {}
    medium_term_trend = vasu.get_medium_trend(train_data)
